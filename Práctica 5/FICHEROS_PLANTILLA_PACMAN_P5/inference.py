...
def constructBayesNet(gameState: hunters.GameState):
    ...
    #Primero creamos un diccionario de las posiciones que alcanzan pacman y los fantasmas y como el tablero es variable
    #se puede mover en cualquier posicion, entonces se crea un rango de 0 a X_RANGE y de 0 a Y_RANGE
    ...
    #En cuanto a las observaciones osea la distancia entre pacman y los fantasmas hay que tener en cuenta todas las distancias hasta el máximo
    #es decir de punta a punta, por eso el X_RANGE y el Y_RANGE y además agragar el ruido del radar MAX_NOISE y restar 1 ya que hay una casilla común
    #cuando pasamos de movernos de vertical a horizontal
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        # Obtener la lista de factores actuales basados en la red bayesiana y la evidencia proporcionada
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        
        # Iterar sobre las variables en el orden de eliminación
        for var in eliminationOrder:
            # Unir los factores actuales por la variable actual
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, var)
            
             # Verificar si el factor resultante tiene más de una variable no condicionada
            length = len(joinedFactor.unconditionedVariables())
            if  length > 1:
                # Realizar la eliminación de la variable actual y agregar el nuevo factor resultante a la lista
                currentFactorsList.append(eliminate(joinedFactor, var))
        
        # Unir todos los factores restantes
        joined = joinFactors(currentFactorsList)
        # Normalizar el factor resultante antes de devolverlo
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        # Calcular la suma total de probabilidades en el factor
        Probabilidad_total = sum(self.values())

        # Verificar si la suma total de probabilidades es cero, en cuyo caso no se puede realizar la normalización
        if Probabilidad_total == 0.0:
            return   # Evitar la normalización si la suma total de probabilidades es cero
        
        # Normalizar las probabilidades dividiendo cada valor por la suma total
        for key in self.keys():
            self[key] /= Probabilidad_total
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        Suma_total = self.total()
        ranNum = random.random() * Suma_total
        distancia = len(self)
        vector = [0] * (distancia + 1)
        vector_auxiliar = [0] * distancia
        i = 0
        vector[0] = 0
        cumulative = 0
        for key in self.keys():
            vector[i + 1] = self[key] + cumulative
            cumulative = vector[i + 1]
            vector_auxiliar[i] = key
            i += 1

        for j in range(distancia):
            if (ranNum >= vector[j] and ranNum < vector[j + 1]):
                return vector_auxiliar[j]
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Se valoran las distintas combinaciones en base a si hay ruido y si el fantasma esta en posicion de carcel,
        #  si ambas son correctas devuelve 1, si hay al menos una correcta da 0
        if noisyDistance == None and ghostPosition == jailPosition:
            ...
            return 0
        # En caso de que ninguna de las dos lo sea correcta, se calcula la probabilidad usando la función getObservationProbability
        else:
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Iterar sobre todas las posiciones posibles en el entorno
        for posicion in self.allPositions:
            # Actualizar la creencia multiplicando por la probabilidad de observación
            self.beliefs[posicion] *= self.getObservationProb(observation, gameState.getPacmanPosition(), posicion, self.getJailPosition())
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
